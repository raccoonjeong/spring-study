...
export function JpaBoardList() {
  ...
  const boardList = async (page) => {
    const res = await fetch(`http://localhost:8080/jpa/board?page=${page}`);
    const data = await res.json();
            ...
            board.map((data) => (
              ...
              >
                ...
                >
                  <input
                    type="checkbox"
                    checked={deleteNums.includes(data.boardNum)}
                    onChange={(e) =>
                      handleDeleteNums(data.boardNum, e.target.checked)
                    }
                  />
                </td>
